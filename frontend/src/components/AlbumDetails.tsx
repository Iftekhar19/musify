...
const AlbumDetails = ({ album, onPlaySong, onLikeSong,songs }: AlbumDetailsProps) => {
  return (
    <div className="flex flex-col gap-6 py-6 h-full">
      ...
      <div className="flex flex-row gap-6 items-start">
        ...
        <div className="flex flex-col gap-2">
          <h1 className="text-3xl font-bold capitalize">{album.title.substring(0,25)}</h1>
          {/* <p className="text-muted-foreground text-sm">{album.artist}</p> */}
          <p className="text-sm mt-2 max-w-lg">{album.description.substring(0,50)}</p>
        </div>
      </div>

      {/* Song List */}
   { songs.length>0?  <Card className="rounded-xl bg-transparent shadow-md border border-border/50">
