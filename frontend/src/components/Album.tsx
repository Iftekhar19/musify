import { Card, CardContent } from "@/components/ui/card";
import { useAuth } from "@/context/AuthProvider";
import type { AlbumCardProps } from "@/types/AllTypes";
import { Trash2 } from "lucide-react";
import { useState } from "react";
import { useNavigate } from "react-router-dom";
import ConfirmDeleteDialog from "./DeleteDialog";
import { Button } from "./ui/button";

const AlbumCard = ({ title, artist, cover, id }: AlbumCardProps) => {
  const navigate = useNavigate();
  const { user } = useAuth();

  const [openDialog, setOpenDialog] = useState(false);
  // const [loading, setLoading] = useState(false);

  const navigatePage = (id: string | number): void => {
    navigate(`/album/${id}`);
  };

  // Delete handler
  const handleDelete = async () => {
    try {
      // setLoading(true);
      // simulate API call
      await new Promise((resolve) => setTimeout(resolve, 1500));
      console.log("Album deleted:", id);
      setOpenDialog(false);
    } catch (err) {
      console.error("Delete failed", err);
    } finally {
      // setLoading(false);
    }
  };

  return (
    <>
      <Card
        onClick={() => navigatePage(id)}
        className="p-0 group cursor-pointer relative max-w-[250px] w-full rounded-2xl overflow-hidden shadow-md hover:shadow-2xl bg-gradient-to-b from-background/80 to-background/40 backdrop-blur-md border border-border/50 transition-all duration-300"
      >
        {user?.role === "admin" && (
          <div className="absolute top-1  right-1 flex items-center gap-2">
            <Button
              onClick={(e) => {
                e.stopPropagation();
                setOpenDialog(true);
              }}
              className="rounded-full bg-red-500  h-8 w-8  z-[50] cursor-pointer hover:bg-red-400"
            >
              <Trash2 className=" text-white" />
            </Button>
            {/* <Button
              onClick={(e) => {
                e.stopPropagation();
                setOpenDialog(true);
              }}
              className="rounded-full   h-8 w-8  z-[50] cursor-pointer 0"
            >
              <PenIcon className=" text-white dark:text-black" />
            </Button> */}
          </div>
        )}

        {/* Album Cover */}
        <div>
          <img
            src={cover}
            alt={title}
            className="h-[150px] w-full object-cover rounded-t-2xl group-hover:scale-105 transition-transform duration-500"
          />
        </div>

        {/* Album Info */}
        <CardContent className="p-4">
          <h3 className="font-semibold truncate text-base">{title}</h3>
          <p className="text-sm text-muted-foreground truncate">{artist}</p>
        </CardContent>
      </Card>

      <ConfirmDeleteDialog
        open={openDialog}
        onOpenChange={setOpenDialog}
        title="Delete Album"
        itemName={title}
        onConfirm={handleDelete}
      />
    </>
  );
};

export default AlbumCard;
